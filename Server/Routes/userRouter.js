  ...
  try {
    ...
    if (!items || !Array.isArray(items) || items.length === 0 || !totalAmount) {
      return res.status(422).json({
        ...responseFormat,
        errorMessage: 'Please add the product into cart before placing order',
      });
    } else {
      // Create new order
      const newOrder = {
      ...
      // Update user's placeOrder array
      userData.orders.push(newOrder);
      await userData.save(); // save updated document
...
// API for User Profile
router.post('/pastOrders', auth, (req, res) => {
  try {
    const userData = req.body.resultDoc;
    res.status(200).json({
      ...responseFormat,
    ...
    res.status(422).json({
      ...responseFormat,
      errorMessage: `Error while calling pastOrders API : ${err}`,
    });
  }
});
